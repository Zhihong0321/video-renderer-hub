...
## 会话进度（2026-06-13）— hyperframes 渲染改逐帧确定性截屏，真 60fps（**已验证，git 待 Joey 定**）

> 起因：Joey 实测导出 MP4 卡顿"从来不是 60fps"。查明根因后把 adapter-hyperframes 的捕获层从实时录屏整个换成逐帧虚拟时钟截屏。README（中英）pipeline 描述已同步更新。

- **根因**：旧实现用 Playwright `recordVideo` = Chromium CDP screencast **实时录屏，硬上限 25fps**；无 GPU 软渲染 1080p 下实测只出 **~5–8 个独立帧/秒**，ffmpeg `-r 60` 只是复制帧填满容器——"60fps"仅是容器标称。`fps` 参数从未影响过真实捕获。
- **新实现**（`packages/adapter-hyperframes/src/render.ts`，`0.3.0-framestep`）：字体等待/动画冻结/时长探测/多 composition 机制全保留，捕获层换成：
  ① `addInitScript` 装一个**惰性虚拟时钟**（`__hvBeginVirtualTime()` 激活前一切走真实时钟，否则字体等待里的 rAF 会自挂）；激活后 rAF 进队列、`performance.now`/`Date.now` 返回虚拟时间、`gsap.ticker.sleep()`；
  ② 每帧 `__hvGoTo(i*1000/fps)`：flush rAF 队列（带虚拟时间戳）→ `gsap.updateRoot()` 手动 tick → `document.getAnimations()` 逐个 `pause()+currentTime=ms`（这条覆盖纯 CSS @keyframes——WAAPI seek 是驱动 CSS 动画的正解，CSS 冻结样式全程不摘）；
  ③ 每帧 `page.screenshot()` PNG（`animations:'allow'` 防 playwright 自己重置动画）→ ffmpeg `-framerate fps -i frame-%06d.png` 直出。旧的 leadIn `-ss` 裁剪 / `tpad` 补尾 / `-r` 重采样**全部删除**——帧序列按构造即精确，explicit/auto 时长都是 totalFrames/fps。
- **验收（量化，daily-report.html 42s@60fps 全程实跑）**：mpdecimate 数独立帧——旧 60/76/43 帧每 10s（三段）→ 新 **216/351/142**；开场动画段 ~44 独立帧/s（余下是 stagger delay 静止段，属内容本身）。容器精确 2520 帧 / 42.000s / 60fps。
- **代价**：~0.3s/帧 wall-time（42s 片 ≈ 12 min）。要快可改 JPEG 截图或降 30fps，约减半。
- **验证方法（以后判流畅度就用这个）**：`ffmpeg -ss X -t 10 -i out.mp4 -vf mpdecimate -an -f null -` 数独立帧；容器 fps 用 ffprobe 看没用，**必须数 mpdecimate 后的帧**。
- **git 待 Joey 定**：render.ts + 双语 README 改动未 commit。根目录另有未追踪的一次性 driver（`render-daily-report.mjs` / `generate-tts.mjs` / `mux-audio.mjs` / `daily-report.html`，不宜进库）。
