...
> **状态：** 可插拔引擎架构已就位，**Hyperframes 引擎已完整接通、能渲染出真实 MP4** —— 无头 Chromium 在步进式虚拟时钟下逐帧截屏带动画的 HTML（每一输出帧都是 i/fps 精确时刻的像素级截图——真 60fps、零丢帧），再用 ffmpeg 编码（libx264）。Remotion、Motion Canvas / Revideo、Manim 在路线图上：适配器接口已为它们设计好，但适配器本身还没写。上表「在 html-video 中」这一列，是当下真正可运行内容的唯一权威来源。
...
| **真实 MP4 渲染** | 无头 Chromium 逐帧截屏带动画的 HTML（真 60fps），ffmpeg 编码（libx264）—— 全在本地，无云端渲染，无单片费用。 |
...
```
  ...
  ④ 逐帧 HTML       每个节点变成一个自包含、带动画的 HTML 帧，落到磁盘
        │
        ▼
  ⑤ Hyperframes 渲染 无头 Chromium 加载每一帧，按虚拟时钟步进到精确的 i/fps
        │            并逐帧截屏（确定性渲染——你要的 fps 就是真实 fps，会自动
        │            延长时长以覆盖该帧自身的动画）→ 每帧一组 PNG 序列
        ▼
  ⑥ ffmpeg          每组 PNG 序列 → mp4（libx264），再 concat 成一支视频；
        │            可选混入 MiniMax 的音乐 + 旁白
        ▼
      你的.mp4
```
...
默认渲染引擎用无头 Chromium 逐帧截屏带动画的 HTML，再用 ffmpeg（libx264）编码为 MP4。如果没有系统安装的 Chromium，装 Playwright 内置的：
